...
<head>
  ...
  <title>Source file /src/pkg/syscall/zsyscall_darwin_386.go</title>
...
<div id="content">
  <h1 id="generatedHeader">Source file /src/pkg/syscall/zsyscall_darwin_386.go</h1>
  ...
  <pre>
<a id="L1"></a><span class="comment">// mksyscall.sh -l32 syscall_darwin.go syscall_darwin_386.go</span>
...
<a id="L252"></a>func Ftruncate(fd int, length int64) (errno int) {
    <a id="L253"></a>_, _, e1 := Syscall(SYS_FTRUNCATE, uintptr(fd), uintptr(length), uintptr(length&gt;&gt;32));
...
<a id="L436"></a>func Pread(fd int, p []byte, offset int64) (n int, errno int) {
    ...
    <a id="L438"></a>if len(p) &gt; 0 {
        <a id="L439"></a>_p0 = &amp;p[0]
    <a id="L440"></a>}
    <a id="L441"></a>r0, _, e1 := Syscall6(SYS_PREAD, uintptr(fd), uintptr(unsafe.Pointer(_p0)), uintptr(len(p)), uintptr(offset), uintptr(offset&gt;&gt;32), 0);
...
<a id="L447"></a>func Pwrite(fd int, p []byte, offset int64) (n int, errno int) {
    ...
    <a id="L449"></a>if len(p) &gt; 0 {
        <a id="L450"></a>_p0 = &amp;p[0]
    <a id="L451"></a>}
    <a id="L452"></a>r0, _, e1 := Syscall6(SYS_PWRITE, uintptr(fd), uintptr(unsafe.Pointer(_p0)), uintptr(len(p)), uintptr(offset), uintptr(offset&gt;&gt;32), 0);
...
<a id="L498"></a>func Seek(fd int, offset int64, whence int) (newoffset int64, errno int) {
    <a id="L499"></a>r0, r1, _ := Syscall6(SYS_LSEEK, uintptr(fd), uintptr(offset), uintptr(offset&gt;&gt;32), uintptr(whence), 0, 0);
    <a id="L500"></a>newoffset = int64(int64(r1)&lt;&lt;32 | int64(r0));
    <a id="L501"></a>return;
<a id="L502"></a>}

<a id="L504"></a>func Select(n int, r *FdSet, w *FdSet, e *FdSet, timeout *Timeval) (errno int) {
    <a id="L505"></a>_, _, e1 := Syscall6(SYS_SELECT, uintptr(n), uintptr(unsafe.Pointer(r)), uintptr(unsafe.Pointer(w)), uintptr(unsafe.Pointer(e)), uintptr(unsafe.Pointer(timeout)), 0);
    <a id="L506"></a>errno = int(e1);
    <a id="L507"></a>return;
<a id="L508"></a>}

<a id="L510"></a>func Setegid(egid int) (errno int) {
    <a id="L511"></a>_, _, e1 := Syscall(SYS_SETEGID, uintptr(egid), 0, 0);
    <a id="L512"></a>errno = int(e1);
    <a id="L513"></a>return;
<a id="L514"></a>}

<a id="L516"></a>func Seteuid(euid int) (errno int) {
    <a id="L517"></a>_, _, e1 := Syscall(SYS_SETEUID, uintptr(euid), 0, 0);
    <a id="L518"></a>errno = int(e1);
    <a id="L519"></a>return;
<a id="L520"></a>}

<a id="L522"></a>func Setgid(gid int) (errno int) {
    <a id="L523"></a>_, _, e1 := Syscall(SYS_SETGID, uintptr(gid), 0, 0);
    <a id="L524"></a>errno = int(e1);
    <a id="L525"></a>return;
<a id="L526"></a>}

<a id="L528"></a>func Setlogin(name string) (errno int) {
    <a id="L529"></a>_, _, e1 := Syscall(SYS_SETLOGIN, uintptr(unsafe.Pointer(StringBytePtr(name))), 0, 0);
    <a id="L530"></a>errno = int(e1);
    <a id="L531"></a>return;
<a id="L532"></a>}

<a id="L534"></a>func Setpgid(pid int, pgid int) (errno int) {
    <a id="L535"></a>_, _, e1 := Syscall(SYS_SETPGID, uintptr(pid), uintptr(pgid), 0);
    <a id="L536"></a>errno = int(e1);
    <a id="L537"></a>return;
<a id="L538"></a>}

<a id="L540"></a>func Setpriority(which int, who int, prio int) (errno int) {
    <a id="L541"></a>_, _, e1 := Syscall(SYS_SETPRIORITY, uintptr(which), uintptr(who), uintptr(prio));
    <a id="L542"></a>errno = int(e1);
    <a id="L543"></a>return;
<a id="L544"></a>}

<a id="L546"></a>func Setprivexec(flag int) (errno int) {
    <a id="L547"></a>_, _, e1 := Syscall(SYS_SETPRIVEXEC, uintptr(flag), 0, 0);
    <a id="L548"></a>errno = int(e1);
    <a id="L549"></a>return;
<a id="L550"></a>}

<a id="L552"></a>func Setregid(rgid int, egid int) (errno int) {
    <a id="L553"></a>_, _, e1 := Syscall(SYS_SETREGID, uintptr(rgid), uintptr(egid), 0);
    <a id="L554"></a>errno = int(e1);
    <a id="L555"></a>return;
<a id="L556"></a>}

<a id="L558"></a>func Setreuid(ruid int, euid int) (errno int) {
    <a id="L559"></a>_, _, e1 := Syscall(SYS_SETREUID, uintptr(ruid), uintptr(euid), 0);
    <a id="L560"></a>errno = int(e1);
    <a id="L561"></a>return;
<a id="L562"></a>}

<a id="L564"></a>func Setrlimit(which int, lim *Rlimit) (errno int) {
    <a id="L565"></a>_, _, e1 := Syscall(SYS_SETRLIMIT, uintptr(which), uintptr(unsafe.Pointer(lim)), 0);
    <a id="L566"></a>errno = int(e1);
    <a id="L567"></a>return;
<a id="L568"></a>}

<a id="L570"></a>func Setsid() (pid int, errno int) {
    <a id="L571"></a>r0, _, e1 := Syscall(SYS_SETSID, 0, 0, 0);
    <a id="L572"></a>pid = int(r0);
    <a id="L573"></a>errno = int(e1);
    <a id="L574"></a>return;
<a id="L575"></a>}

<a id="L577"></a>func Settimeofday(tp *Timeval) (errno int) {
    <a id="L578"></a>_, _, e1 := Syscall(SYS_SETTIMEOFDAY, uintptr(unsafe.Pointer(tp)), 0, 0);
    <a id="L579"></a>errno = int(e1);
    <a id="L580"></a>return;
<a id="L581"></a>}

<a id="L583"></a>func Setuid(uid int) (errno int) {
    <a id="L584"></a>_, _, e1 := Syscall(SYS_SETUID, uintptr(uid), 0, 0);
    <a id="L585"></a>errno = int(e1);
    <a id="L586"></a>return;
<a id="L587"></a>}

<a id="L589"></a>func Stat(path string, stat *Stat_t) (errno int) {
    <a id="L590"></a>_, _, e1 := Syscall(SYS_STAT64, uintptr(unsafe.Pointer(StringBytePtr(path))), uintptr(unsafe.Pointer(stat)), 0);
    <a id="L591"></a>errno = int(e1);
    <a id="L592"></a>return;
<a id="L593"></a>}

<a id="L595"></a>func Statfs(path string, stat *Statfs_t) (errno int) {
    <a id="L596"></a>_, _, e1 := Syscall(SYS_STATFS64, uintptr(unsafe.Pointer(StringBytePtr(path))), uintptr(unsafe.Pointer(stat)), 0);
    <a id="L597"></a>errno = int(e1);
    <a id="L598"></a>return;
<a id="L599"></a>}

<a id="L601"></a>func Symlink(path string, link string) (errno int) {
    <a id="L602"></a>_, _, e1 := Syscall(SYS_SYMLINK, uintptr(unsafe.Pointer(StringBytePtr(path))), uintptr(unsafe.Pointer(StringBytePtr(link))), 0);
    <a id="L603"></a>errno = int(e1);
    <a id="L604"></a>return;
<a id="L605"></a>}

<a id="L607"></a>func Sync() (errno int) {
    <a id="L608"></a>_, _, e1 := Syscall(SYS_SYNC, 0, 0, 0);
    <a id="L609"></a>errno = int(e1);
    <a id="L610"></a>return;
<a id="L611"></a>}

<a id="L613"></a>func Truncate(path string, length int64) (errno int) {
    <a id="L614"></a>_, _, e1 := Syscall(SYS_TRUNCATE, uintptr(unsafe.Pointer(StringBytePtr(path))), uintptr(length), uintptr(length&gt;&gt;32));
    <a id="L615"></a>errno = int(e1);
    <a id="L616"></a>return;
<a id="L617"></a>}

<a id="L619"></a>func Umask(newmask int) (errno int) {
    <a id="L620"></a>_, _, e1 := Syscall(SYS_UMASK, uintptr(newmask), 0, 0);
    <a id="L621"></a>errno = int(e1);
    <a id="L622"></a>return;
<a id="L623"></a>}

<a id="L625"></a>func Undelete(path string) (errno int) {
    <a id="L626"></a>_, _, e1 := Syscall(SYS_UNDELETE, uintptr(unsafe.Pointer(StringBytePtr(path))), 0, 0);
    <a id="L627"></a>errno = int(e1);
    <a id="L628"></a>return;
<a id="L629"></a>}

<a id="L631"></a>func Unlink(path string) (errno int) {
    <a id="L632"></a>_, _, e1 := Syscall(SYS_UNLINK, uintptr(unsafe.Pointer(StringBytePtr(path))), 0, 0);
    <a id="L633"></a>errno = int(e1);
    <a id="L634"></a>return;
<a id="L635"></a>}

<a id="L637"></a>func Unmount(path string, flags int) (errno int) {
    <a id="L638"></a>_, _, e1 := Syscall(SYS_UNMOUNT, uintptr(unsafe.Pointer(StringBytePtr(path))), uintptr(flags), 0);
    <a id="L639"></a>errno = int(e1);
    <a id="L640"></a>return;
<a id="L641"></a>}

<a id="L643"></a>func Write(fd int, p []byte) (n int, errno int) {
    <a id="L644"></a>var _p0 *byte;
    <a id="L645"></a>if len(p) &gt; 0 {
        <a id="L646"></a>_p0 = &amp;p[0]
    <a id="L647"></a>}
    <a id="L648"></a>r0, _, e1 := Syscall(SYS_WRITE, uintptr(fd), uintptr(unsafe.Pointer(_p0)), uintptr(len(p)));
    <a id="L649"></a>n = int(r0);
    <a id="L650"></a>errno = int(e1);
    <a id="L651"></a>return;
<a id="L652"></a>}

<a id="L654"></a>func read(fd int, buf *byte, nbuf int) (n int, errno int) {
    <a id="L655"></a>r0, _, e1 := Syscall(SYS_READ, uintptr(fd), uintptr(unsafe.Pointer(buf)), uintptr(nbuf));
    <a id="L656"></a>n = int(r0);
    <a id="L657"></a>errno = int(e1);
    <a id="L658"></a>return;
<a id="L659"></a>}

<a id="L661"></a>func write(fd int, buf *byte, nbuf int) (n int, errno int) {
    <a id="L662"></a>r0, _, e1 := Syscall(SYS_WRITE, uintptr(fd), uintptr(unsafe.Pointer(buf)), uintptr(nbuf));
    <a id="L663"></a>n = int(r0);
    <a id="L664"></a>errno = int(e1);
    <a id="L665"></a>return;
<a id="L666"></a>}

<a id="L668"></a>func gettimeofday(tp *Timeval) (sec int32, usec int32, errno int) {
    <a id="L669"></a>r0, r1, e1 := Syscall(SYS_GETTIMEOFDAY, uintptr(unsafe.Pointer(tp)), 0, 0);
    <a id="L670"></a>sec = int32(r0);
    <a id="L671"></a>usec = int32(r1);
    <a id="L672"></a>errno = int(e1);
    <a id="L673"></a>return;
<a id="L674"></a>}
</pre>
